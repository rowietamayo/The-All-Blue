    ...
    <div className="container mx-auto px-4 py-12">
      <div className="text-center mb-12 animate-in slide-in-from-bottom-4 duration-500">
        ...
        <p className="text-muted-foreground max-w-2xl mx-auto">
          The brilliant minds bringing the All Blue to life.
        </p>
      </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {[1, 2, 3, 4].map((i) => (
            <Card
              key={i}
              className="animate-pulse flex flex-col sm:flex-row overflow-hidden"
            >
        ...
        <div className="text-center py-20 border border-dashed border-border rounded-xl">
          ...
          <p className="text-muted-foreground">
            The crew is currently out at sea gathering ingredients.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {chefs.map((chef) => (
            <Card
              key={chef.id}
              className="flex flex-col sm:flex-row overflow-hidden hover:shadow-lg transition-shadow group"
            >
              {chef.imageUrl ? (
